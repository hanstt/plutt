...
MyTest g_test_node_mean_arith_;
...
{
  {
    ...
    TEST_CMP(v.GetEnd().at(0), ==, 2UL);
    TEST_CMP(v.GetV(0, true), ==, (1 + 3) / 2);
    TEST_CMP(v.GetV(1, true), ==, 2 / 1);
  }
  {
    MockNode0 nv0(Input::kUint64, 1);
    MockNode1 nv1(Input::kUint64, 1);
    NodeMeanArith n("", &nv0, &nv1);
    ...
    TEST_CMP(v.GetEnd().at(0), ==, 2UL);
    TEST_CMP(v.GetEnd().at(1), ==, 3UL);
    TEST_CMP(v.GetEnd().at(2), ==, 4UL);
    TEST_CMP(v.GetV().size(), ==, 4UL);
    TEST_CMP(v.GetV(0, true), ==, (1 + 5) / 2);
    TEST_CMP(v.GetV(1, true), ==, (2 + 6) / 2);
    TEST_CMP(v.GetV(2, true), ==, 3);
    TEST_CMP(v.GetV(3, true), ==, 7);
  }
}
